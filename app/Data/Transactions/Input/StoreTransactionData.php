...
class StoreTransactionData extends BaseInputData
{
    /**
    ...
    {
        return [StoreTransactionRequestNormalizer::class, ...parent::normalizers()];
    }
    ...
    public static function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if (! $user instanceof User || ! $ledger instanceof Ledger || $user->isPremium()) {
                return;
            }
            ...
            if ($accountId !== 0 && ! $allowedAccountIds->contains($accountId)) {
                $validator->errors()->add('account_id', 'This account is not available on the free plan.');
            }

            $toAccountId = (int) request()->input('to_account_id');
            if ($toAccountId !== 0 && ! $allowedAccountIds->contains($toAccountId)) {
...
class StoreTransactionRequestNormalizer implements Normalizer
{
