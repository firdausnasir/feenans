...
class ImportController extends Controller
{
    ...
    public function parse(ParseImportRequest $request, Ledger $ledger): JsonResponse
    {
        ...
        if ($file === null || ! $file->isValid() || $file->getRealPath() === false || $file->getRealPath() === '') {
            return response()->json([
                ...
                'errors' => ['file' => ['The uploaded file could not be read. Please upload it again.']],
            ], 422);
        }
    ...
    public function execute(StoreImportRequest $request, Ledger $ledger): JsonResponse
    {
        ...
        if (! Storage::disk($this->ledgerDisk())->exists($filePath)) {
            return response()->json([
                'message' => 'Import file not found. Please re-upload.',
                'errors' => ['file_path' => ['Import file not found. Please re-upload.']],
            ], 422);
        }
        ...
        if ($stream === null) {
            return response()->json([
                'message' => 'Import file could not be read. Please re-upload.',
                'errors' => ['file_path' => ['Import file could not be read. Please re-upload.']],
            ], 422);
        }
